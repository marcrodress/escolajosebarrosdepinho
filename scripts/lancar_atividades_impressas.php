...
while($res_atividades = mysqli_fetch_array($sql_atividades)){
	...
	$sql_verifica_envio = mysqli_query($conexao_bd, "SELECT * FROM atividades_enviadas WHERE code_aluno = '$aluno' AND code_atividade = '$code_atividade'");
	 if(mysqli_num_rows($sql_verifica_envio) == ''){
		...
		mysqli_query($conexao_bd, "INSERT INTO atividades_enviadas (data, status, arquivo, code_aluno, code_atividade, nota, presente) VALUES ('$data', 'CORRIGIDO', '', '$aluno', '$code_atividade', '$nota', 'SIM')");	
	 }
}
...
$sql_notas_bimestrais = mysqli_query($conexao_bd, "SELECT * FROM notas_bimestrais WHERE aluno = '$aluno' AND bimestre = '$bimestre' AND componente = '$componente' AND turma = '$turma'");
if(mysqli_num_rows($sql_notas_bimestrais) == ''){
	...
	$media = 0;
	 ...
	 while($res_notas_bimestrais = mysqli_fetch_array($sql_notas_bimestrais)){
		 $media = $res_notas_bimestrais['media'];
	}
	
	if($media < 6){
	mysqli_query($conexao_bd, "UPDATE notas_bimestrais SET tipo = 'COORDENACAO', media = '$nota', at = '$nota', ap = '$nota', ab = '$nota' WHERE aluno = '$aluno' AND bimestre = '$bimestre' AND componente = '$componente'");
	}
}
}
