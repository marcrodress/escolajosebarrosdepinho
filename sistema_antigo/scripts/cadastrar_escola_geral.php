...
if($nome == ''){
	...
	$sql_verifica = mysqli_query($conexao_bd, "SELECT * FROM acesso_sistema WHERE usuario = '$login'");
	...
	$sql_verifica = mysqli_query($conexao_bd, "SELECT * FROM acesso_sistema WHERE email = '$email'");
		if(mysqli_num_rows($sql_verifica) >= 1){
		 echo "<script language='javascript'>window.alert('Já existe um cadastro com esse e-mail em sistema!');</script>";
			...
			$code = rand();
			mysqli_query($conexao_bd, "INSERT INTO acesso_sistema (code, nome_escola, login, senha, img, email, tipo) VALUES ('$code', '$nome', '$login', '$senha', '', '$email', 'ESCOLA')");
