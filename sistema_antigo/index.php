	 ...
	 <? } ?>
	  ...
	  if(mysqli_num_rows($sql_verifica) == ''){
	   echo "<div class='alert alert-danger' role='alert'>Aluno não encontrado, verifique seu nome e digite novamente!</div>";
		...
		echo "<script language='javascript'>window.location='?p=2&turma=$turma&ik=$id&aluno=$aluno&disciplina=';</script>";      
	  }}} ?>
      <form method="post" action="" enctype="multipart/form-data">
      <p class="h5"><strong>DIGITE SEU NOME</strong></p>
...
<? } ?>  
  ...
  <? if($_GET['onde'] == 'app'){ ?>
  <li><a rel="superbox[iframe][340x250]" href="professor/arquivos/<? echo $res_arquviso['arquivo']; ?>"><? echo $i; ?> - <? if($res_arquviso['nome_arquivo'] == ''){ echo "Baixar material de apoio"; }else{ echo $res_arquviso['nome_arquivo']; }?></a></li>
  <? }else{ ?>
  ...
  <div class="row">
    <div class="col-sm">
     ...
     <? if($res['tipo_envio'] == 'arquivo'){ ?>
  	  <p class="h5 text-danger"><strong>Enviar atividade</strong> - <a class="btn btn-primary" href="envio/index.php?aluno=<? echo $_GET['aluno']; ?>&atividade=<? echo $code_atividade; ?>">Modelo antigo</a></p><hr />
      <?
     ...
     <a href="?p=2&turma=<? echo $_GET['turma']; ?>&ik=<? echo $_GET['ik']; ?>&aluno=<? echo $_GET['aluno']; ?>&disciplina=<? echo $_GET['disciplina']; ?>&pg=excluir&id=<? echo $res_atividades['id']; ?>"><img src="professor/img/deleta.fw.png" width="10" height="10" border="0" /></a></li>
     </ul>
	 <? } ?>
