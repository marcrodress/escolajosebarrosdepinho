...
if($id > $total_atividades){
		echo "<script language='javascript'>window.location='enviar_sms.php';</script>";
}else{
	
	$sql_atividade = mysqli_query($conexao_bd, "SELECT * FROM atividades WHERE id = '$id' AND code_entrega2 = '$code_hoje'");
	if(mysqli_num_rows($sql_atividade) == ''){ $id++;
	 ...
	 $data_atividade = "$dia_atividade/$mes_atividade/$ano_atividade";
	  ...
	  $componente = 0;
	  $sql_componente = mysqli_query($conexao_bd, "SELECT * FROM turmas WHERE code_turma = '$turma_atividade'");
	  ...
	  $sql_professor = mysqli_query($conexao_bd, "SELECT * FROM professor WHERE operador = '$professor'");
	   while($res_professor = mysqli_fetch_array($sql_professor)){
		...
		$nome_aluno = 0;
		$sql_alunos_turma = mysqli_query($conexao_bd, "SELECT * FROM alunos WHERE turma = '$turma_atividade' AND telefone != ''");
