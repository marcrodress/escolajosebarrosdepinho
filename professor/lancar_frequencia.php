...
while($res_atividade = mysqli_fetch_array($sql_atividade)){
	...
	if(mysqli_num_rows($sql_verifica) == ''){
		mysqli_query($conexao_bd, "INSERT INTO atividades_enviadas (data, status, arquivo, code_aluno, code_atividade, nota, presente) VALUES ('$data', 'CORRIGIDO', '$operador', '$aluno', '".$res_atividade['code_atividade']."', '10', 'SIM')");
	}else{
...
while($res_atividade = mysqli_fetch_array($sql_atividade)){
	...
	mysqli_query($conexao_bd, "INSERT INTO atividades_enviadas (data, status, arquivo, code_aluno, code_atividade, nota, presente) VALUES ('$data', 'CORRIGIDO', '', '$aluno', '".$res_atividade['code_atividade']."', '10', 'SIM')");
    ...
    <div class="container">
      <div class="row">
        ...
        <table class="table table-striped">
          ...
          <?
          $k = 0;
		  $sql_turma_alunos = mysqli_query($conexao_bd, "SELECT * FROM turmas_alunos WHERE turma = '".$_GET['turma']."' AND laudado = 'SIM'");
		   if(mysqli_num_rows($sql_turma_alunos) == ''){
          ...
          <?  while($res_alunos = mysqli_fetch_array($sql_alunos)){ $k++; ?>
            <tr>
              ...
              <td><? echo $res_alunos['nome_aluno']; ?> | <a rel="superbox[iframe][380x380]" href="scripts/lancar_atividades_impressas.php?aluno=<?  echo $res_turma_alunos['aluno']; ?>&turma=<? echo $_GET['turma']; ?>"><img src="img/notas_finais.png" width="20" height="20" /></a></td>
			  <? for($i=1; $i<=31; $i++){ if($i<10){  $dia_t = "0$i"; }else{  $dia_t = $i; }
				  ...
				  <a href="?p=lancar_frequencia&turma=<? echo $_GET['turma']; ?>&dia=<? echo $dia_t; ?>&mes=<? echo $_GET['mes']; ?>&ano=<? echo $ano; ?>&acao=ativargg&aluno=<? echo $res_alunos['code_aluno']; ?>"><img src="img/correto_atividade.png" width="15" height="15" border="0" title="Lançar frequência deste dia" /></a>	 
				  <? }else{ ?>
				  <a href="?p=lancar_frequencia&turma=<? echo $_GET['turma']; ?>&dia=<? echo $dia_t; ?>&mes=<? echo $_GET['mes']; ?>&ano=<? echo $ano; ?>&acao=excluir&aluno=<? echo $res_alunos['code_aluno']; ?>"><img src="img/errado_atividade.png" width="15" height="15" border="0" title="Excluir frequência desse aluno neste dia" /></a>	 						
				  <? } ?>
          </td>
                  <? } ?>
