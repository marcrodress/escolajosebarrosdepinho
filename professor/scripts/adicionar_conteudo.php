...
if($id == NULL){
$sql_eixo = mysqli_query($conexao_bd, "SELECT * FROM habilidades WHERE conteudo = '$eixo' AND ano = '".$_GET['ano']."' AND componente = '".$_GET['componente']."'");
 if(mysqli_num_rows($sql_eixo) == ''){
	mysqli_query($conexao_bd, "INSERT INTO habilidades (status, conteudo, ano, componente, eixo, cod_habilidade, habilidade) VALUES ('Ativo', '$eixo', '".$_GET['ano']."', '".$_GET['componente']."', '', '', '')");
...
if($id >= 1){
	...
	mysqli_query($conexao_bd, "UPDATE habilidades SET conteudo = '$eixo' WHERE id = '$id'");
