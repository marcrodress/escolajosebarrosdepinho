...
<script>
	function confirmacao_deleta(aluno, matricula) {
		var confirmacao = confirm("Deseja realmente excluir essa ação?");
		if(confirmacao === true){
			 window.location.href = "?aluno="+aluno+"&p=cancelar_matricula&matricula="+matricula;
		}
	}
...
<table width="721" border="0">
  ...
  <tr>
    ...
    <a href="?aluno=<? echo $_GET['aluno']; ?>&p=aee_tirar&laudado=&matricula=<? echo $res_alunos['code_matricula']; ?>"><img style="border-radius:10px; border:1px solid #000;" src="../img/infrequencia.png" width="7" height="7" border="0" title="Tirar o aluno do AEE" /></a>
    <? } ?>
    </td>
    ...
    <? } ?>
      ...
      <a href="javascript:func()" onclick="confirmacao_deleta('<? echo $_GET['aluno']; ?>', '<? echo $res_alunos['code_matricula']; ?>')"><img src="../img/deleta.jpg" width="20" height="20" border="0" title="Exclusão realiazada com sucesso!" /></a>
...
$sqlCancela = mysqli_query($conexao_bd, "UPDATE turmas_alunos SET status = 'CANCELADO' WHERE code_matricula = '".$_GET['matricula']."'");
if($sqlCancela == ''){
echo "<script language='javascript'>alert('Ocorreu um erro!');window.location='?aluno=$aluno';</script>";
}else{
...
<h1 style="font:12px Arial, Helvetica, sans-serif;"><strong>Informação laudo do aluno</strong></h1>
<form action="" method="post" enctype="multipart/form-data">

<table width="410" border="0">
  <tr>
    <td width="42"><strong>CID</strong></td>
    <td width="144"><strong>DESCRIÇÃO</strong></td>
    <td width="144"><strong>OBSERVAÇÕES</strong></td>
    <td width="52">&nbsp;</td>
  </tr>
  <tr>
    <td><label for="textfield"></label>
    <input name="cid" type="text" id="textfield" size="7"></td>
    <td><label for="textfield2"></label>
    <input type="text" name="descricao" id="textfield2"></td>
    <td><label for="textfield3"></label>
    <input type="text" name="observacoes" id="textfield3"></td>
    <td><input type="submit" name="button" id="button" value="Enviar"></td>
  </tr>
  </table>
</form>
...
if($cid == ''){
	echo "<script language='javascript'>window.alert('Digite o número do CID');</script>";
}elseif($descricao == ''){
	echo "<script language='javascript'>window.alert('Digite a descrição do CID');</script>";
}elseif($observacoes == ''){
	echo "<script language='javascript'>window.alert('Digite as observações do aluno, comportamentos, anotações, etc...');</script>";
}else{
	mysqli_query($conexao_bd, "UPDATE turmas_alunos SET laudado = 'SIM' WHERE aluno = '$aluno'");
	mysqli_query($conexao_bd, "INSERT INTO aee_descricao (operador, cid, descricao, observacao, aluno) VALUES ('$operador', '$cid', '$descricao', '$observacoes', '$aluno')");
...
<? if($_GET['p'] == 'aee_tirar'){
	...
	mysqli_query($conexao_bd, "UPDATE turmas_alunos SET laudado = '' WHERE aluno = '$aluno'");
	mysqli_query($conexao_bd, "DELETE FROM aee_descricao WHERE aluno = '$aluno'");
