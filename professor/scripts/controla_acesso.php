...
<table width="392" border="1">
  <tr>
    <td width="29" bgcolor="#0099CC"><strong>COD.</strong></td>
    <td width="49" bgcolor="#0099CC"><strong>STATUS</strong></td>
    <td width="36" bgcolor="#0099CC"><strong>LOGIN</strong></td>
    <td width="51" bgcolor="#0099CC"><strong>ACESSO</strong></td>
    <td width="193" bgcolor="#0099CC">&nbsp;</td>
  </tr>
  ...
  <tr>
    ...
    <td>
    	<a href="javascript:func()" onclick="confirmacao('<? echo $res['cpf']; ?>', '<? echo $res_cpf['code']; ?>', 'Excluido')"><img src="../../img/deleta.png" width="20" height="20" title="Deletar acesso" /></a>
        
        <? if($res_cpf['status'] == 'Ativo'){ ?>
        <a href="javascript:func()" onclick="confirmacao('<? echo $res['cpf']; ?>', '<? echo $res_cpf['code']; ?>', 'Inativo')"><img src="../../img/pausa.png" width="20" height="20" title="Bloquear acesso" /></a>
        <? } ?>
        
        <? if($res_cpf['status'] == 'Inativo'){ ?>
        <a href="javascript:func()" onclick="confirmacao('<? echo $res['cpf']; ?>', '<? echo $res_cpf['code']; ?>', 'Ativo')"><img src="../../img/ativar.png" width="20" height="20" title="Ativar acesso" /></a>
        <? } ?>
...
<script>
  function confirmacao(cpf, login, status){
	 var confirmacao = confirm("Tem que deseja fazer essa operação");
	 if(confirmacao === true){
			 window.location.href = "?cpf="+cpf+"&login="+login+"&p=1&acao="+status;
		}
 }
</script>
...
<? if(@$_GET['p'] == '1'){
	...
	$sqlUpdate = mysqli_query($conexao_bd, "UPDATE acesso_sistema SET status = '$acao' WHERE code = '$login'");
	 if($sqlUpdate == ''){
		echo "<script language='javascript'>alert('Erro');</script>";
	 }else{
	 
	 $sqlBuscaCpf = mysqli_query($conexao_bd, "SELECT * FROM acesso_sistema WHERE code = '$login'");
	 	 while($resCPF = mysqli_fetch_array($sqlBuscaCpf)){
			 $cpf = $resCPF['cpf'];
		}
		echo "<script language='javascript'>alert('$sqlUpdate');</script>";
