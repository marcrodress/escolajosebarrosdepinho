...
<div class="container_tuod"> 
    ...
    <div class="container">
      <div class="row" style="font:10px Arial, Helvetica, sans-serif;">
        ...
        <p class="h5 text-primary"><strong>Planos de aula aguardando visto - ANOS INICIAIS</strong></p>
        <hr />
        ...
        <table border="0" class="table">
           <thead class="thead-dark">
			 ...
			 ?>
                      <option value="?p=plano_de_aula_pendente_iniciais&ano=<? echo $res_turmas['code_turma']; ?>&componente=<? echo $_GET['componente']; ?>&mes=<? echo $_GET['mes']; ?>"><? echo $res_turmas['code_serie']; ?>° ANO - TURMA: <? echo $res_turmas['tipo_turma']; ?> - TURNO: <? echo $res_turmas['turno']; ?></option>
                      <? } ?>
                    </select>
                  </form>
                </form></td>
    	  ...
    	  <?
		   $sql_verifica = mysqli_query($conexao_bd, "SELECT * FROM plano_de_aula_iniciais WHERE turma = '".$_GET['ano']."' AND status_coordenacao != 'VISTO' AND status_coordenacao != 'CORRECAO'");
              ...
              <tr>
                <td align="center"><? 
					...
					if(mysqli_num_rows($sql_atividade) == ''){
						mysqli_query($conexao_bd, "DELETE FROM plano_de_aula_iniciais WHERE id_aula = '".$res_plano_final['id_aula']."'");
					 ...
					 while($res_acesso_sistema = mysqli_fetch_array($sql_acesso_sistema)){
					  ...
					  $sql_coladorares = mysqli_query($conexao_bd, "SELECT * FROM coladorares WHERE cpf = '".$res_acesso_sistema['cpf']."'");
					    while($res_coladorares = mysqli_fetch_array($sql_coladorares)){						
						 ...
						 echo $res_coladorares['nome'];
					}
					 }
				?>                
                </td>
                <? }}} ?>
                <td align="left"><? echo $res_plano_final['objetivo']; ?></td>
                <td align="center">
                ...
                <a rel="superbox[iframe][405x250]" href="scripts/rejeitar_plano_inicial.php?id=<? echo $res_plano_final['id']; ?>"><img src="img/errado_atividade.png" width="20" height="20" border="0" title="Rejeitar " /></a>
                
                
                <a target="_blank" href="pdf/imprimir_plano_de_aula_iniciais.php?componente=<? echo $componente; ?>&turma=<? echo $_GET['ano']; ?>&professor=<? echo $professor; ?>&tipo=arquivo&ik=<? echo $ik; ?>"><img src="img/impressora2.png" width="20" height="20" border="0" /></a>
...
<? if(@$_GET['acao'] == 'visto'){
	...
	mysqli_query($conexao_bd, "UPDATE plano_de_aula_iniciais SET status_coordenacao = 'VISTO' WHERE id = '".$_GET['id']."'");
	
	$turma = $_GET['turma'];
		
	echo "<script language='javascript'>window.location='?p=plano_de_aula_pendente_iniciais&ano=$turma';</script>";
