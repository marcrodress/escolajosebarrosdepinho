...
$sql = mysqli_query($conexao_bd, "SELECT * FROM acesso_sistema WHERE login = '$login' AND senha = '$senha' AND status = 'Ativo'");
if(mysqli_num_rows($sql) == ''){
	...
	while($res = mysqli_fetch_array($sql)){
		$sql_colaborador = mysqli_query($conexao_bd, "SELECT * FROM coladorares WHERE cpf = '".$res['cpf']."'");
			while($res_colaborador = mysqli_fetch_array($sql_colaborador)){
				$nome_colaborador = $res_colaborador['nome'];	
			}
		
	session_start();
	$_SESSION['code'] = $res['code'];	
	$_SESSION['nome'] = $nome_colaborador;	
	...
	if($senha == '12345'){
		echo "<script language='javascript'>window.alert('Por questões de segurança, inativamos seu acesso, atualize sua senha na próxima tela para que seu acesso seu acesso volte a ficar ativo');</script>";
		mysqli_query($conexao_bd, "UPDATE acesso_sistema SET status = 'Inativo' WHERE login = '$login' AND senha = '$senha'");
	}
	
	if($res['tipo'] == 'AEE'){
	echo "<script language='javascript'>window.location='index.php?p=aee';</script>";
	}elseif($res['tipo'] == 'PROFESSOR'){
